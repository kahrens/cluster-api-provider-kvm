...
```sh
make docker-build docker-push IMG=<some-registry>/cluster-api-provider-kvm:tag
```
...
```sh
make deploy IMG=<some-registry>/cluster-api-provider-kvm:tag
```
...
```sh
make build-installer IMG=<some-registry>/cluster-api-provider-kvm:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/cluster-api-provider-kvm/<tag or branch>/dist/install.yaml
```
